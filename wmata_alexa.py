...
def on_launch(launch_request, session):
    print("on_launch requestId={}, sessionId={}".format(launch_request['requestId'],
                                                                 session['sessionId']))
    return get_welcome_response()


def on_intent(intent_request, session):
    print("on_intent requestId={}, sessionId={}".format(intent_request['requestId'],
                                                                 session['sessionId']))
...
def query_station(station, destination = None):
    if station.lower() == "dulles":
            return "dulles"
    station_data = get_stations()
    st_options = get_options(station, station_data)

    if destination is not None:
        (intersection, st_code, st_index, dest_trajectory) = destination_proc(station_data, st_options, station, destination)
...
def destination_proc(station_data, st_options, station, destination):
    ...
    return intersection, st_code, st_index, dest_trajectory
